...
const SAFE={
  ...
  aliment:'pomme poire banane orange citron raisin fraise cerise noix noisette pois riz pain lait café thé soupe fromage oeuf œuf gâteau sucre sel miel radis chou tomate carotte salade viande'.split(' '),
  ...
  outil:'scie vis pelle marteau tournevis clé pince hache couteau fourche aiguille clou perceuse'.split(' '),
...
function candidate(r,{label,description,pos,provenance,provenanceStatus,confidence,concreteCategory=null,visualConceptCandidate=false,evidence=[],rationale='',ambiguityNotes=[]}){return {senseId:stableSenseId(r,label,provenanceStatus),label,description,pos:pos||r.pos||null,provenance,provenanceStatus,confidence,concreteCategory,visualConceptCandidate,evidence,rationale,ambiguityNotes};}
function inferredCandidate(r){
  ...
  const guarded=SYMBOL_RE.test(w)||(r.pos==='NOM'&&ABSTRACT_SUFFIX.test(w)&&!cat);
  const visual=Boolean(cat)&&!guarded;
  return candidate(r,{label:r.exactWord,description:visual?`Sens concret candidat « ${r.exactWord} » (${cat}), à confirmer au niveau concept.`:`Sens lexical « ${r.exactWord} » non résolu comme concept visuel autonome.`,pos:r.pos,provenance:'sense-prototype/automatic-semantic-guard-v1',provenanceStatus:'inferred_candidate',confidence:visual?0.82:(r.pos==='NOM'?0.38:0.22),concreteCategory:cat,visualConceptCandidate:visual,evidence:[`exact_graphy_ipa:${r.exactWord}|${r.ipa}`,cat?`safe_concrete_lexicon:${cat}`:'no_positive_concrete_sense_evidence',guarded?'generic_guard':''],rationale:visual?'Catégorie concrète explicite et autonome dans le lexique de garde.':'Un POS nominal ou une fréquence élevée ne suffit pas à établir un concept visuel.',ambiguityNotes:guarded?['symbolic or morphology-level abstract risk']:[]});
}
function observedCandidates(r){const out=[];for(const e of r.legacyEvidence||[]){if(!e?.source)continue;const rejected=String(e.decision||'').includes('reject');out.push(candidate(r,{label:r.conceptLabel||r.exactWord,description:r.conceptDescription||`Concept structuré historique pour ${r.exactWord}.`,pos:r.pos,provenance:`rebulo_structured:${e.source}`,provenanceStatus:'observed',confidence:rejected?0.25:0.82,concreteCategory:r.conceptCategory||null,visualConceptCandidate:!rejected&&['low','medium','unknown'].includes(e.namingRisk||'unknown'),evidence:[`structured_record:${e.source}`,`decision:${e.decision||'mentioned'}`],rationale:'Information de sens/concept déjà présente dans une source structurée Rebulo; elle est observée comme donnée source, sans devenir preuve de nommage humain.',ambiguityNotes:e.mainConfusions||[]}));}return out;}
...
const enriched=corpus.map((r,i)=>{const senseCandidates=[inferredCandidate(r),...observedCandidates(r)];const rec=recMap.get(key(r));if(rec)senseCandidates.push(editorialCandidate(r,rec));const automaticSerious=senseCandidates.some(s=>s.provenanceStatus==='inferred_candidate'&&s.visualConceptCandidate&&s.confidence>=0.8);const evidenceBackedSerious=senseCandidates.some(s=>['observed','editorial_added'].includes(s.provenanceStatus)&&s.visualConceptCandidate&&s.confidence>=0.8);const afterSerious=automaticSerious||evidenceBackedSerious;const green=evidenceBackedSerious;return {calibrationId:`SC-${String(i+1).padStart(4,'0')}`,ipa:r.ipa,exactWord:r.exactWord,lexicalEntries:lexicalEntriesOf(r),baseline:{provisionalClass:r.provisionalClass||r.baselineClass||null,aggregateScore:r.aggregateScore??r.baselineAggregate??null,serious:['A','B'].includes(r.provisionalClass||r.baselineClass)},calibrationGroup:r.calibrationGroup,goldSerious:r.goldSerious,goldSource:r.goldSource,senseCandidates,automaticSenseGateSerious:automaticSerious,evidenceBackedSerious,afterSenseResolutionSerious:afterSerious,greenRouteEligible:green,humanNamingEvidence:'none'};});
...
const green=enriched.filter(r=>r.greenRouteEligible).length;const orange=enriched.filter(r=>r.afterSenseResolutionSerious&&!r.greenRouteEligible).length;const red=enriched.length-green-orange;
const result={schemaVersion:'1.1',status:'analysis_only',baseSha:BASE_SHA,corpusSize:enriched.length,labeledEvaluationSize:labeled.length,formula290:'unchanged',humanNamingEvidence:'none',senseCandidateSchema:['senseId','label','description','pos','provenance','provenanceStatus','confidence','concreteCategory','visualConceptCandidate','evidence','rationale','ambiguityNotes'],provenanceStatusDefinitions:{observed:'information réellement présente dans une source structurée',editorial_added:'sens ajouté après revue éditoriale explicite',inferred_candidate:'proposition automatique; jamais assimilée à une vérité observée'},counts,routes:{green,orange,red},automaticSenseGateSelected:enriched.filter(r=>r.automaticSenseGateSerious).length,afterSenseResolutionSelected:enriched.filter(r=>r.afterSenseResolutionSerious).length,unresolvedCases:red,greenRouteEligible:green,before:metrics(r=>r.baseline.serious),automaticSenseGate:metrics(r=>r.automaticSenseGateSerious),afterSenseResolution:metrics(r=>r.afterSenseResolutionSerious),greenRouteOnLabeled:metrics(r=>r.greenRouteEligible),sample2KnownABFalsePositiveRatePercent:s2Audit.falsePositiveRatePercent,examples:{knownFalsePositives:s2Audit.falsePositives.map(x=>x.exactWord),rescuedConcepts:recovered.map(x=>x.exactWord)},notes:['Primary precision/recall are computed only on independently labeled cases from complete #292 A/B audit plus #291/#292 rescue reviews and near-miss reviews.','The automatic semantic guard does not contain the known #292 false-positive token list and does not contain the eight rescue concepts as positive exceptions.','Pilot #290 and other sample rows are stress/calibration proxies and are excluded from primary gold metrics.','No inferred_candidate is converted to observed; green route requires evidence-backed observed/editorial_added sense evidence.']};
