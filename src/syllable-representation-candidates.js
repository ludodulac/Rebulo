...
export function wholeWordRepresentationCandidates(ipa='',entries=[],limit=12){
  const target=normalizeIPA(ipa);
  if(!target)return [];
  const best=new Map();
  for(const raw of entries||[]){
    if(normalizeIPA(raw?.ipa||'')!==target)continue;
    const word=String(raw?.word||'').trim();
    const lemma=String(raw?.lemma||word).trim();
    const pos=lexicalPos(raw?.pos);
    if(!word||!lemma||!(pos in POS_PRIORITY))continue;
    if(!/^[\p{L}'’\-]+$/u.test(word))continue;
    const key=`${normalizedWord(lemma)}|${target}`;
    const candidate={
      word,
      lemma,
      ipa:target,
    ...
    const isLemmaForm=normalizedWord(word)===normalizedWord(lemma);
    const previousIsLemma=previous?normalizedWord(previous.word)===normalizedWord(previous.lemma):false;
    if(!previous||candidate.lexicalPriority<previous.lexicalPriority||
      (candidate.lexicalPriority===previous.lexicalPriority&&isLemmaForm&&!previousIsLemma)||
      (candidate.lexicalPriority===previous.lexicalPriority&&isLemmaForm===previousIsLemma&&candidate.frequency>previous.frequency)){
      best.set(key,candidate);
    }
  }
  const sorted=[...best.values()].sort((a,b)=>
    a.lexicalPriority-b.lexicalPriority||b.frequency-a.frequency||a.word.localeCompare(b.word,'fr')
  );
