...
const target={therapy:['denomination','syllable-blending','oral-to-written','phoneme-deletion','unknown-target']};
const activities=buildTherapyActivities(target,definitions);
assert.deepEqual(activities.map(item=>item.id),['denomination','syllable-blending','oral-to-written']);
...
assert.match(activityInstruction(activities[1],'child'),/nom entier de chaque image/);
assert.match(activityInstruction(activities[1],'pro'),/sans suppression ni substitution/);
