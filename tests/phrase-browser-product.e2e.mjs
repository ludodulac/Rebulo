...
async function runPhrase(key,value,{cold=false}={}){
  ...
  if(requireVisible)await page.locator('#creatorForm button[type="submit"]').click();else await input.press('Enter');
...
await page.locator('[data-creator-kind="word"]').click();await page.waitForFunction(()=>document.querySelector('.app-shell')?.dataset.creatorKind==='word');await page.locator('#target').fill('merci');await page.locator('#creatorForm button[type="submit"]').click();await page.waitForFunction(()=>!document.querySelector('#result')?.hidden&&document.querySelector('#resultWord')?.textContent?.toLowerCase().includes('merci'),null,{timeout:30000});report.productChecks.createWord=await page.evaluate(()=>({result:document.querySelector('#resultWord')?.textContent?.trim()||'',pieces:[...document.querySelectorAll('#creatorRebus .piece')].map(node=>node.textContent?.trim()||'').filter(Boolean)}));
await page.locator('#playMode').click();await page.waitForFunction(()=>document.querySelector('.app-shell')?.dataset.experience==='play'&&!document.querySelector('#playArena')?.hidden);await page.waitForFunction(()=>document.querySelectorAll('#playRebus .piece').length>0,null,{timeout:30000});report.productChecks.play=await page.evaluate(()=>({pieceCount:document.querySelectorAll('#playRebus .piece').length,catalogSize:Number(document.querySelector('.app-shell')?.dataset.playCatalogSize||0),bank:document.querySelector('.app-shell')?.dataset.playBank||''}));
