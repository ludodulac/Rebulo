...
const target={targetIpa:'/mɛʁsi/',therapy:['denomination','lexical-access','phoneme-initial','phoneme-final','phoneme-segmentation','phoneme-blending','syllable-blending','oral-to-written','phoneme-deletion','unknown-target']};
const activities=buildTherapyActivities(target,definitions);
assert.deepEqual(activities.map(item=>item.id),['denomination','lexical-access','phoneme-initial','phoneme-final','phoneme-segmentation','phoneme-blending','syllable-blending','oral-to-written']);
...
assert.match(activityInstruction(activities[6],'child'),/nom entier de chaque image/);
assert.match(activityInstruction(activities[6],'pro'),/sans suppression ni substitution/);
