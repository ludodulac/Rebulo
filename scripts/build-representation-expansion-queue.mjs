...
const lines=[
  ...
  `- Sons avec asset dont l’IPA est explicitement enregistrée et exacte : ${stats.registeredPhoneticAssetSoundCount}.`,
  ...
  '',
  `- Assets existants à examiner : ${subqueues.existingAssetReview.length} sons (${subqueues.existingAssetReview.slice(0,20).map(x=>`/${x}/`).join(', ')}).`,
  ...
  '',
  '| Rang | Son | Cibles utiles | Fenêtre | Candidats exacts retenus | Convention | Assets par libellé | File de décision |',
  '|---:|---|---:|---|---|---|---:|---|',
  ...top.map((row,index)=>`| ${index+1} | /${row.ipa}/ | ${row.usefulTargetCount} | ${row.syllableSpans.join('/')||'—'} syll. | ${candidateText(row)} | ${row.visibleConventions.map(x=>x.label).join(', ')||'—'} | ${row.existingAssetCandidateCount} | ${laneLabel[row.lane]||row.lane} |`),
  '',
  ...
  '- Un asset existant déclenche une inspection avant dessin; un simple nom de fichier de recherche ne devient pas une preuve phonétique ou de dénomination.',
