...
import {buildExplicitDeletionOperation,buildExplicitSubstitutionOperation,buildGeneralConstruction,buildGraphemeOperation,buildSpatialRelationOperation,buildStrictConstruction,buildWholeWordOperation,REBUS_OPERATION_TYPES} from './rebus-construction.js';
...
export function buildCreatorCandidate(target,lexicon=[],therapyDefinitions=[]){
  ...
  const candidate={answer:target.target,targetIpa:target.targetIpa,syllableCount:Number.isInteger(target.syllableCount)&&target.syllableCount>0?target.syllableCount:null,source:target.source||'',generated:Boolean(target.generated),pieces:pieces.map(piece=>({...piece,reading:piece.label})),therapyActivities:buildTherapyActivities(target,therapyDefinitions)};
