...
const soundSearchForm=document.getElementById('soundSearchForm');
...
let lexicalEntries=[];let lexicalSource='';let productivityById=new Map();let productivitySource='';let submittedSoundQuery='';
...
function currentSounds(){return resolveFrenchSoundQuery(submittedSoundQuery);}
...
function render(){
  ...
  if(!sound.query)interpretation.textContent='Écris un son comme tu le prononces, puis appuie sur « Chercher ce son ».';
...
function submitSoundSearch(){submittedSoundQuery=soundInput.value;render();results.scrollIntoView?.({behavior:'smooth',block:'start'});}
soundSearchForm.addEventListener('submit',event=>{event.preventDefault();submitSoundSearch();});
...
for(const example of frenchSoundHelp()){const button=document.createElement('button');button.type='button';button.textContent=example;button.addEventListener('click',()=>{soundInput.value=example;submitSoundSearch();});chips.appendChild(button);}
for(const control of [positionSelect,queryInput,strictOnly])control.addEventListener(control.tagName==='SELECT'||control.type==='checkbox'?'change':'input',render);
render();loadContext();
