...
import {generatedPlayableBankRebuses,generatedPlayableRebuses,mergePlayableCatalog,playCatalogMetrics} from '../src/generated-play-catalog.js';
...
if(serializedComplete)assert.equal(generated.length,coverage.strictMultiPieceUniqueWordCount,'complete analysis output must expose every strict multi-image word to legacy play generation');
else assert.ok(generated.length>=300,'legacy truncated report should still expose substantially more than the 21 historical rounds before regeneration');
console.log('PLAY_VISIBLE_DIVERSITY '+JSON.stringify({legacy:legacyMetrics,modern:modernMetrics,profiles:profileMetrics}));
