...
function evaluate(pieces,{examplesLimit=20}={}){
...
function scenario(name,added){
  const next=evaluate([...base,...added],{examplesLimit:200});
  const addedLabels=new Set(added.map(item=>item.label));
  const examples=next.examples.filter(example=>example.pieces.some(label=>addedLabels.has(label))).slice(0,12);
  return {
    name,
    ...
    examples,
    note:added.length===1?'Gain isolé par rapport à la base active actuelle.':'Gain combiné recalculé ; il ne faut pas additionner les gains isolés.'
  };
}
