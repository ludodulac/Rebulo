...
const report={
  constructible:[
    {word:'merci',ipa:'mɛʁsi',lemma:'merci',pos:'NOM',frequency:100,syllableCount:2,syllabification:'mɛʁ.si',sourceSyllabification:'mɛʁ.si',decomposition:['mer','scie']},
    ...
    {word:'auras',ipa:'oʁa',lemma:'avoir',pos:'VER',frequency:4,syllableCount:2,syllabification:'o.ʁa',sourceSyllabification:'o.ʁa',decomposition:['eau','rat']}
  ],
  missingSounds:[
    {ipa:'te',examples:[
      {word:'cité',ipa:'site',lemma:'cité',pos:'NOM',frequency:80,frame:['scie','[te]']},
      {word:'théière',ipa:'tejɛʁ',frequency:20,frame:['[te]','hier']},
      {word:'mixte',ipa:'site',frequency:500,frame:['scie','[te]']}
    ]},
    {ipa:'su',examples:[{word:'souris',ipa:'suʁi',frequency:60,frame:['[su]','riz']},{word:'sous-lit',ipa:'suli',frequency:5,frame:['[su]','lit']}]},
    {ipa:'ɛʁ',examples:[{word:'merci-R',ipa:'mɛʁsiɛʁ',frequency:5,frame:['mer','scie','[ɛʁ]']}]},
...
assert.deepEqual(targets.map(item=>item.target),['merci','cinéma','sans-compte','mixte','auras']);
assert.equal(targets[0].mode,'strict');
assert.equal(targets[0].assets,'ready');
assert.equal(targets[0].syllableCount,2);
assert.equal(targets[0].lemma,'merci');
assert.equal(targets[0].pos,'NOM');
assert.equal(targets[0].sourceSyllabification,'mɛʁ.si');
assert.deepEqual(targets[0].therapy,['denomination','lexical-access','phoneme-initial','phoneme-final','phoneme-segmentation','phoneme-blending','syllable-count','syllable-segmentation','oral-to-written']);
...
const auras=targets.find(item=>item.target==='auras');
assert.equal(auras.lemma,'avoir','an inflected form must retain its source lemma for downstream lexical reasoning');
assert.equal(auras.pos,'VER');
assert.equal(auras.sourceSyllabification,'o.ʁa');
...
assert.equal(graphemeTargets[0].lemma,'cité');
assert.equal(graphemeTargets[0].pos,'NOM');
assert.deepEqual(graphemeTargets[0].operations,[{type:'whole_word',pieceId:'scie'},{type:'grapheme',grapheme:'T',reading:'té'}]);
...
assert.deepEqual(spatialTargets[0].operations,[{type:'spatial_relation',relation:'under',reading:'sous'},{type:'whole_word',pieceId:'riz'}]);
...
const ranked=rankCreatorTargets([{target:'x',mode:'general',frequency:1000,operationCount:2},{target:'x',mode:'strict',frequency:1,operationCount:4},{target:'x',mode:'general',frequency:10,operationCount:3}]);
...
assert.equal(automatic.find(item=>item.target==='auras').lemma,'avoir');
...
const generated=[targets.find(item=>item.target==='merci'),targets.find(item=>item.target==='cinéma'),{target:'refus',targetIpa:'/mɛʁsi/',syllableCount:2,mode:'strict',assets:'ready',therapy:['syllable-count'],generated:true},{target:'generated-only',targetIpa:'/ʒenere/',syllableCount:3,mode:'strict',assets:'ready',therapy:['syllable-count'],generated:true},...graphemeTargets,...spatialTargets,targets.find(item=>item.target==='mixte')].filter(Boolean);
const merged=mergeCreatorTargets(manual,generated);
const mergedMerci=merged.find(item=>item.target==='merci');
assert.deepEqual(mergedMerci.therapy,['denomination','syllable-count','syllable-segmentation','syllable-blending','oral-to-written'],'manual syllable blending must be preserved while generated syllable metadata may supplement it');
const mergedRefus=merged.find(item=>item.target==='refus');assert.equal(mergedRefus.mode,'rejected');assert.equal(mergedRefus.alternatives,undefined);
const mergedCite=merged.find(item=>item.target==='cité');assert.equal(mergedCite.manualNote,'manual general wins');assert.equal(mergedCite.alternatives,undefined,'identical generated construction must be deduplicated');
const mergedSouris=merged.find(item=>item.target==='souris');assert.equal(mergedSouris.manualNote,'manual spatial wins');assert.equal(mergedSouris.alternatives,undefined,'identical spatial generation must not duplicate the manual pilot');
const mergedMixte=merged.find(item=>item.target==='mixte');assert.equal(mergedMixte.manualNote,'manual primary wins');assert.equal(mergedMixte.alternatives.length,1);assert.equal(mergedMixte.alternatives[0].mode,'general');
