...
assert.ok(session);assert.equal(session.sessionCode,'S01');assert.deepEqual(session.candidateIds,['dos-a','dos-b']);assert.deepEqual(session.observations,[]);assert.equal(createNamingSession(comparison,{sessionCode:''}),null);
...
assert.equal(exported.sessionCode,'S01');assert.equal(exported.observations.length,2);assert.match(exported.researchNotice,/No automatic activation or clinical validation/);assert.equal('startedAt' in exported,false);assert.equal('participantName' in exported,false);
...
for(const conceptName of ['pot','dos','raie','tas','terre']){
  const liveComparison=registry.comparisons.find(item=>item.concept===conceptName);assert.ok(liveComparison);assert.equal(liveComparison.candidates.length,4);
...
console.log('naming test runner: neutral trials, twenty stimuli and failed-image capture guardrails ok');
