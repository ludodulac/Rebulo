...
const first=potReview.candidates.find(item=>item.candidateId===ids[0]);assert.equal(first.observationCount,2);assert.equal(first.targetResponseCount,2,'case-normalized exact concept matches should be counted descriptively');assert.deepEqual(first.responses,[{response:'pot',count:2}]);
const second=potReview.candidates.find(item=>item.candidateId===ids[1]);assert.equal(second.hesitationCount,1);assert.equal(second.targetResponseCount,0);
...
assert.match(js,/buildNamingObservationReview/);assert.match(js,/Comptages descriptifs uniquement/);assert.doesNotMatch(js,/localStorage|sessionStorage|method\s*:\s*['"]POST/i);assert.doesNotMatch(js,/clinical_approved|active\s*[:=]\s*true/i);
assert.match(namingHtml,/naming-review\.html/);
console.log('naming observation review: strict anonymous import validation and descriptive counts only.');
