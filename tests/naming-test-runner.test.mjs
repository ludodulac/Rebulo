...
const sol=productionRegistry.reviews.find(item=>item.concept==='sol');assert.ok(sol);assert.equal(sol.revision,'sol-note-v2');assert.equal(sol.targetIpa,'/sɔl/');assert.equal(sol.activationState,'active_general_naming_review');assert.equal(sol.automaticActivation,false);assert.deepEqual(sol.candidates[0].namingRisks,['note','note de musique','musique']);assert.match(sol.candidates[0].reviewNote,/problème de dénomination/);await access(new URL(`../${sol.candidates[0].asset}`,import.meta.url));
const solSeed=seed.find(item=>item.id==='sol');assert.ok(solSeed);assert.equal(solSeed.artRevision,'sol-note-v2');assert.equal(solSeed.assetSource,'rebulo_original:sol-note-v2');assert.equal(solSeed.clinicalStatus,'naming_test_required');assert.equal(solSeed.active,true);
const solSource=assetSources.assets.find(item=>item.path==='assets/rebus/sol.svg');assert.ok(solSource);assert.equal(solSource.source,'rebulo_original');assert.equal(solSource.sourceCommit,'4cb0c773ed9046558396dafa2b304a6e1227caca');assert.equal(solSource.artRevision,'sol-note-v2');assert.equal(solSource.clinicalStatus,'naming_test_required');
...
console.log('naming test runner: inactive prototypes plus pluie, clé and sol production reviews stay revision-bound, anonymous and non-clinical.');
