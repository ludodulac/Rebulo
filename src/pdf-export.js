...
export function sanitizeFilePart(value='rebulo'){
  const normalized=String(value||'rebulo').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9]+/g,'-').replace(/^-+|-+$/g,'');
  return normalized||'rebulo';
}

export function worksheetCopy(rebus,mode='pro'){
  const pieces=rebus?.pieces||[];
  if(mode==='child')return {title:'Quel mot entends-tu ?',subtitle:'Prononce chaque image puis écris ta réponse.',showAnswer:false,showLabels:false,showIpa:false,answerLine:true};
  return {title:rebus?.answer||'Correction',subtitle:'Correction orthophonique - dénominations entières et concaténation phonétique stricte.',showAnswer:true,showLabels:true,showIpa:true,answerLine:false,proof:`${pieces.map(piece=>piece.ipa).join(' + ')} = ${rebus?.targetIpa||''}`};
}
...
async function imageUrlToPng(src,size=520){const image=await loadImage(src);const canvas=document.createElement('canvas');canvas.width=size;canvas.height=size;const context=canvas.getContext('2d');context.fillStyle='#ffffff';context.fillRect(0,0,size,size);const scale=Math.min(size/image.naturalWidth,size/image.naturalHeight)*0.82;const width=image.naturalWidth*scale;const height=image.naturalHeight*scale;context.drawImage(image,(size-width)/2,(size-height)/2,width,height);return canvas.toDataURL('image/png');}
...
async function drawWorksheetPage(doc,rebus,{mode='pro',metadata={},pageNumber=1,pageCount=1}={}){
  ...
  drawCenteredText(doc,copy.title,42,{size:22,style:'bold'});drawCenteredText(doc,copy.subtitle,52,{size:10,color:[95,105,122]});
  const count=rebus.pieces.length;const gap=count>=4?4:7;const usable=174;const cardWidth=Math.min(count>=4?40:48,(usable-gap*(count-1))/count);const total=cardWidth*count+gap*(count-1);const startX=(210-total)/2;const cardY=72;const cardHeight=72;const imageSize=Math.min(34,cardWidth-8);
