...
assert.equal(Object.keys(REBULO_INDIVIDUAL_VISUALS).length,23,'visible batch migration should expose exactly the 23 currently reviewed individual assets');
assert.deepEqual(REBULO_VISIBLE_BATCH1.filter(item=>!item.individualImage).map(item=>item.id),['bebe','mer'],'unresolved concepts must keep the reviewed sprite fallback set');
