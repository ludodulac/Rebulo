...
function session(sessionCode,responses){
  return {schemaVersion:'1.0',sessionCode,concept:'pot',comparisonRevision:pot.revision,targetIpa:pot.targetIpa,candidateIds:[...ids],observations:ids.map((candidateId,index)=>({candidateId,responseVerbatim:responses[index]?.response||'',hesitation:responses[index]?.hesitation===true,noResponse:responses[index]?.noResponse===true})),researchNotice:'Raw anonymous naming observations only. No automatic activation or clinical validation.'};
}
...
const {comparisonRevision,...legacy}=s1;assert.equal(validateNamingObservationExport(legacy,comparisons),null,'unstamped legacy exports must be refused rather than guessed');
