...
assert.equal(isDrawableNamingCandidate({word:'qu'}),false);
assert.equal(isDrawableNamingCandidate({word:'est'}),false);
...
assert.equal(ranked.some(x=>x.ipa==='t'),false,'non drawable grammatical fragments must not reach the shortlist');
...
assert.equal(ranked.find(x=>x.label==='thé').nextGate,'human_drawability_review');
