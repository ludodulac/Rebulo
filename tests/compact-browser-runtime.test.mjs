...
assert.deepEqual(compact.creatorTargets,expectedCreatorTargets,'compiled creator targets must exactly match canonical generated creator targets');
assert.ok(compact.bankRows.length>0&&compact.playRebuses.length>0&&compact.creatorTargets.length>0);
console.log(`compact-browser-runtime.test.mjs: ${compact.bankRows.length} rows / ${compact.playRebuses.length} play rebuses / ${compact.creatorTargets.length} creator targets`);
