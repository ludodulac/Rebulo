...
for(const target of usefulTargets){
  ...
  for(let start=0;start<syllables.length;start++){
    for(let length=1;length<=2&&start+length<=syllables.length;length++){
      ...
      if(!row){row={ipa,syllableSpans:new Set(),usefulTargetCount:0,usefulWeightedGain:0,minAgeBandCandidate:12,examples:[],candidateMap:new Map()};rows.set(ipa,row);}
...
const report={generatedAt:new Date().toISOString(),status:'research_only',policyVersion:policy.version||null,source:{name:lexique.source||'Lexique 4',license:lexique.license||null,entryCount:entries.length},stats,rows};
...
const lines=[
  ...
  '',
  `- Cibles utiles analysées : ${stats.usefulTargetCount}.`,
  `- Fenêtres utiles distinctes : ${stats.usefulSoundWindowCount}.`,
  `- Fenêtres avec au moins un mot entier approximatif admissible : ${stats.approximationCandidateWindowCount}.`,
